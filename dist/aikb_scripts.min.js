(() => {
  ...
  function extractWorkAreas(results) {
    ...
    results.forEach((result) => {
      if (result.listMetadata && result.listMetadata.keyword && result.listMetadata.keyword[0]) {
  ...
  function addDropdownIcons() {
    ...
    if (workAreaDropdown && !workAreaDropdown.parentElement.querySelector(".aikb-dropdown-icon")) {
      ...
      icon.innerHTML = '<span class="fal fa-chevron-down" aria-hidden="true"></span>';
  ...
  function initializeDropdowns(results) {
    ...
    "src/js/ntg-funnelback.js"() {
      ...
      ntgFunnelback = {
        ...
        callSearchAPI: function(query) {
          ...
          if (ntgFunnelback.filteredterm && ntgFunnelback.filteredterm.trim().length > 0) {
            ...
            error: function(xhr, status, error) {
              ...
              $.ajax({
                ...
                error: function() {
                  ...
                  if (container) {
                    container.innerHTML = "<p>Unable to load search results. Please try again later.</p>";
                  }
                }
              });
            }
        ...
        filterQuery: function() {
          if (!ntgFunnelback.originalterm || ntgFunnelback.originalterm.trim() === "") {
        ...
        processResults: function(dataset) {
          ...
          console.log(
            `After filtering: ${filteredResults.length} results (excluded ${results.length - filteredResults.length})`
          );
  ...
  function loadInitialResults() {
    ...
    fetch("src/data/search.json").then((response) => {
      ...
      searchResultsContainer.innerHTML = '<p class="aikb-error">Error loading search data: ' + fallbackError.message + "</p>";
    });
    const apiURL = "https://ntgov-search.funnelback.squiz.cloud/s/search.json?collection=ntgov~sp-ntgc-ai-knowledge-base&s=!FunDoesNotExist:PadreNull";
