(() => {
  ...
  function tokenizeQuery(query) {
    ...
    const tokens = normalized.split(/\s+/).filter((token) => token.length > 0 && !NOISE_WORDS.has(token));
    return tokens;
  }
  ...
  function searchLocalData(keywords, results) {
    ...
    if (matchedResults.length > 0) {
      ...
      matchedResults.slice(0, 3).forEach((result, index) => {
        console.log(
          `  ${index + 1}. "${result.title}" (score: ${result._offlineScore.toFixed(2)}, matched: [${result._matchedTerms.join(", ")}], raw: ${result._rawScore}, len: ${result._docLength})`
        );
      });
    }
